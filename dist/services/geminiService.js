...
Ensure the JSON is properly formatted with double quotes around all property names and string values.`;
    ...
    validateAndCorrectTechniques(techniques) {
    ...
    validateAndCorrectFeatures(features) {
    ...
    validateAndCorrectStorageOptions(storageOptions) {
        ...
        for (const storage of requiredStorage) {
            if (storageOptions[storage] && typeof storageOptions[storage] === 'string') {
                correctedStorage[storage] = storageOptions[storage];
            }
        }
        return correctedStorage;
    }
    async renameIngredient(ingredient) {
        const maxRetries = 3;
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
    ...
    createIngredientRenamingPrompt(ingredient) {
...
Ensure the JSON is properly formatted with double quotes around all property names and string values.`;
     ...
     */
    async updateIngredientType(ingredient) {
    ...
    createIngredientTypeClassificationPrompt(ingredient) {
...
Do not include any explanations, quotes, or additional text.`;
    ...
    parseIngredientTypeResponse(response) {
        try {
            // Clean the response
    ...
    fallbackIngredientTypeClassification(ingredient) {
